...
export const Button = ({ tipo, texto = '', icon: Icon, disabled = false, onClick }: Props) => {
    ...
    if (tipo === 'primary') {
        return (
            <button
                onClick={onClick}
                disabled={disabled} className={`border flex w-full justify-center gap-2 items-center rounded-lg   px-2 py-1 font-semibold  text-white cursor-pointer hover:opacity-90`}
                style={{ backgroundColor: rotiseriaActive?.color, borderColor: rotiseriaActive?.color }}
            >
